...
import {
  CrewMemberFragment,
  CrewUpdateActionEnum,
  RoleEnum,
  useCrewQuery,
  useJoinCrewMutation,
  useKickMemberMutation,
  useLeaveCrewMutation,
  useTransferCrewOwnershipMutation,
...
export const CrewScreen: React.FC = () => {
  ...
  const transferCrewOwnership = useCallback(
    (to: string) => {
      transferOwnership({
        variables: {
          to
  ...
  const updateMember = useCallback(
    (action: CrewUpdateActionEnum, id: string) => {
      if (data?.crew?.id) {
        update({
          variables: {
            action,
            ...
            cache.modify({
              ...
              fields: {
                ...
                members(existings: StoreObject[], { readField }) {
                  return existings.filter((ref) => id !== readField('id', ref))
                }
              }
            })
          }
          ...
          <>
            <View style={tailwind('mb-5')}>
              <View style={tailwind('flex-row')}>
                <ProgressiveImage
                  ...
                  style={tailwind(
                    'w-28 h-28 rounded-xl mb-1 bg-white-200 dark:bg-black-200'
                  )}
                  resizeMode="cover"
                />
            ...
            {isAwaitingResponse && (
              <Text style={tailwind('text-green-300 -mt-2 mb-5')}>
                ...
                has been noticed, you will receive an update if you have been
                getting accepted!
              </Text>
            )}
            ...
            <View style={tailwind('mb-5')}>
              ...
              <Participant
                ...
                onPress={() =>
                  navigate('UserProfile', { id: data?.crew?.admin.id })
                }
              />
            </View>

            <Text style={tailwind('text-xl font-bold')}>
              Members ({membersWithoutLeader?.length})
            </Text>
          </>
        }
        renderItem={({ item }) => (
